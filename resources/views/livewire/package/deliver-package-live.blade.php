<div>
                    ...
                    <x-mary-table :headers="$headers" :rows="$encomiendas" with-pagination per-page="perPage"
                        ...
                        @scope('cell_remitente', $stuff)
                            <div class="grid grid-cols-1 grid-rows-4 gap-1 text-xs">
                                ...
                                <div>
                                    <x-mary-badge :value="$stuff->sucursal_remitente->name" class="text-xs text-white bg-green-500" />
                                </div>
                                <div>
                                    <x-mary-badge :value="$stuff->sucursal_remitente->created_at->format('d/m/Y')" class="text-xs text-right text-white badge-warning" />
                                </div>
                                <div>
                        ...
                        @scope('cell_destinatario', $stuff)
                            <div class="grid grid-cols-1 grid-rows-4 gap-1 text-xs">
                                ...
                                <div>
                                    <x-mary-badge :value="$stuff->sucursal_destinatario->name" class="text-xs text-white bg-green-500" />
                                </div>
                                <div>
                                    <x-mary-badge :value="$stuff->sucursal_destinatario->created_at->format('d/m/Y')" class="text-xs text-right text-white badge-warning" />
                                </div>
                                <div>
                                    @if ($stuff->isHome)
                                        REPARTO DOMICILIO
    ...
    @isset($encomienda)
        ...
        <x-mary-drawer wire:model="showDrawer" title="Detalle de encomienda" subtitle="Code {{ $encomienda->code }}"
            ...
            <x-mary-card shadow>
                ...
                <div class="grid grid-cols-5 grid-rows-3 gap-1 bg-red-100 rounded">
                    ...
                    <div class="row-start-2">
                        {{ strtoupper($encomienda->destinatario->type_code) ?? 'type_code' }}</div>
                        ...
                        <div class="grid col-span-8">
                            <x-mary-card shadow>
                                ...
                                <div class="grid grid-cols-5 grid-rows-3 gap-1 bg-green-200 rounded">
                                    ...
                                    <div class="row-start-2">{{ $encomienda->remitente->code ?? 'code' }}</div>
                                    <div class="row-start-2">{{ $encomienda->remitente->phone ?? 'phone' }}</div>
                                    <div class="col-span-3">{{ $encomienda->sucursal_remitente->name ?? 'sucursal' }}
                                    </div>
                                </div>
                                <x-mary-icon name="s-envelope" class="text-red-500 text-md" label="DESTINATARIO" />
                                <div class="grid grid-cols-5 grid-rows-3 gap-1 bg-red-100 rounded">
                        ...
                        <div class="grid col-span-8 space-y-2">
                            <x-mary-card shadow>
                                ...
                                @if ($encomienda->monto_descuento)
                                    <x-mary-button label="QUITAR DESCUENTO" icon="o-currency-dollar"
                                        wire:click="descuentoDelete({{ $encomienda->id }})" spinner
                                        class="mt-2 w-full text-white bg-purple-500 btn-xs" />
                                    ...
                                    <x-mary-radio class="w-full max-w-full py-0 text-xs" :options="$comprobantes"
                                        ...
                                        disabled />
                                @else
                                    <x-mary-button label="ASIGNAR DESCUENTO" icon="o-currency-dollar"
                                        wire:click="descuento({{ $encomienda->id }})" spinner
                                        class="mt-2 w-full text-white bg-purple-500 btn-xs" />
                                            ...
                                            <div class="grid grid-cols-4 gap-2 p-2 border rounded-lg border-sky-500">
                                                <div class="grid col-span-4 pt-2">
                                                    <x-mary-input label="Numero de documento"
                                                        wire:model.live='customerFact.code'>
                                                        <x-slot:prepend>
                                                            @php
                                                            ...
                                                            @endphp
                                                            <x-mary-select wire:model.live='customerFact.type_code'
                                                                icon="o-user" :options="$docsfact"
                                                                class="rounded-e-none" />
                                                        </x-slot:prepend>
                                                        <x-slot:append>
                                                            <x-mary-button wire:click='searchFacturacion'
                                                ...
                                                <div class="grid col-span-2 pt-2">
                                                    <x-mary-input label="Nombre/Raz. Social"
                                                        wire:model.live='customerFact.name'>
                                                    </x-mary-input>
                                                </div>
                                                <div class="grid col-span-2 pt-2">
                                                    <x-mary-input label="Direccion"
                                                        wire:model.live='customerFact.address'>
                                                    </x-mary-input>
                                                </div>
                                            </div>
                                        @endif
        ...
        <x-mary-modal wire:model="modalDescuento" persistent class="backdrop-blur" box-class="max-h-full max-w-md">
            <x-mary-card shadow separator class="border border-blue-500">
                <x-mary-form wire:submit.prevent="descuentoCreate">
