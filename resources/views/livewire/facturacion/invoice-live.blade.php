<div>
    <x-mary-card title="{{ $title }}" subtitle="{{ $sub_title }}" separator>
        <x-slot:menu>
            <x-mary-button icon="o-document-chart-bar" wire:click="refresh" spinner
        ...
        @endphp
        <x-mary-table :headers="$headers" :rows="$invoices" striped with-pagination per-page="perPage" :per-page-values="[5, 20, 10, 50]">
            ...
            @scope('cell_menu', $stuff)
                <x-mary-dropdown>
                    ...
                    <x-mary-menu-item title="Estado SUNAT" icon="o-archive-box" wire:click="statusInvoice({{ $stuff->id }})" />
