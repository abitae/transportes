            ...
            <div class="flex items-center justify-between">
                ...
                <div class="flex items-center">
                    <div class="text-xs text-center">
                        <img src="{{ env('APP_URL') }}/{{ $invoice->company->logo_path }}" alt="Logo" class="w-48 h-18">
                        <p>{{ $invoice->company->address }}</p>
                        <p>Email:{{ $invoice->company->email }}- Telf: {{ $invoice->company->telephone }}</p>
                    </div>
                </div>
                <!-- Título de la Factura, Serie y Correlativo en un recuadro -->
                <div class="bordered-box">
                    <h1 class="text-lg">R.U.C. {{ $invoice->company->ruc }}</h1>
                    <h1 class="text-lg font-semibold">{{ $invoice->tipoDoc ? 'FACTURA ELECTRONICA' : 'BOLETA
                        ELECTRONICA' }}</h1>
                    <p class="text-md">{{ $invoice->serie }}-{{ $invoice->correlativo }}</p>
                </div>
            </div>
        </header>
